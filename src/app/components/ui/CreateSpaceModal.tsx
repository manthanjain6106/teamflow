...
export default function CreateSpaceModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const submit = async () => {
    if (!name.trim() || !selectedWorkspace?.id) return;
    setSaving(true);
    try {
      await createSpace({ name: name.trim(), workspaceId: selectedWorkspace.id });
      onClose();
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
        <div className="p-4 border-b border-gray-200 dark:border-gray-800 text-lg font-semibold">Create space</div>
        <div className="p-4 space-y-3">
          <div>
            <label className="text-sm text-gray-600 dark:text-gray-300">Name</label>
            <input value={name} onChange={(e) => setName(e.target.value)} className="mt-1 w-full px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800" placeholder="Space name" />
          </div>
        </div>
