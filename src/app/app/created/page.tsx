...
export default function CreatedByMePage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchTasks({ createdById: 'me' });
      setTasks(data || []);
    } catch (e: any) {
  ...
  return (
    <div className="flex-1 bg-gray-50 dark:bg-gray-900 overflow-y-auto">
      <div className="max-w-6xl mx-auto p-6">
        <h1 className="text-2xl font-bold mb-4 flex items-center gap-2"><Plus className="h-6 w-6 text-purple-600" /> Created by me</h1>
        {tasks.length === 0 ? <div className="text-gray-500">No tasks created by you.</div> : (
          <div className="space-y-2">
            {tasks.map((t) => (
              <div key={t.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded p-3">
                ...
                <div className="text-xs text-gray-500">by {t.createdBy?.name || 'You'}</div>
              </div>
            ))}
          </div>
        )}
